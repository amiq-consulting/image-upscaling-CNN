...
#pragma HLS INTERFACE axis register both port = stream_out
    ...
    /*if you change the input image be aware that the second dimension
     of the buffer must be the width of the input image + PADDING_NUMBER*2*/
    static package_t buffer[NR_OF_NEEDED_ROWS_PER_PIXEL][WIDTH_MAX];
    //#pragma HLS ARRAY_PARTITION variable=buffer complete dim=1  //the latency is increased (x10),
    // but BRAM is reduced (x3).
...
L_width_read_line_of_pixels:
            for(int k=0; k<width; k++)
            {
//#pragma HLS LOOP_TRIPCOUNT min=64 max=64
#pragma HLS PIPELINE //the latency is increased
//#pragma HLS UNROLL //NO because of stream_in
...
//#pragma HLS LOOP_TRIPCOUNT min=128 max=128
//#pragma HLS PIPELINE
//#pragma HLS UNROLL//NO because of stream_out
            x = j * h + 2;
            x1 = 1 + x - floor(x);
            x2 = x - floor(x);
            x3 = floor(x) + 1 - x;
